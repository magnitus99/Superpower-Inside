...
| UI를 바꿨는가 | 가능한 한 커밋 전에 실제 Obsidian 화면이나 충실한 시뮬레이션을 스크린샷으로 확인합니다. 밀도, 정렬, 대비, overflow, 상태 변화를 코드나 테스트 결과로 대신하지 않습니다. 화면 검수 없이 UI/UX 완료로 보고하지 않습니다. |
...
- UI/DOM/CSS/레이아웃/카피/상태 표시를 바꾸면 가능한 한 커밋 전에 실제 실행 화면을 스크린샷으로 확인합니다. 릴리스 절차와 별개로 화면 검수 없는 UI/UX 완료 보고는 금지합니다.
...
UI 변경의 화면 검수는 가능한 한 커밋 전에 끝냅니다. 릴리스 절차 자체에 별도 스크린샷 단계를 추가하지 않으며, 화면 검수가 빠졌다면 UI/UX 완료로 보고하지 않습니다.
