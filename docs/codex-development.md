...
이 저장소의 개발 지침과 스킬은 GPT-6 Astra를 기준으로 정리했지만 특정 모델을 요구하지 않습니다. 필수 작업 계약은 [AGENTS.md](../AGENTS.md), 세부 개발 흐름은 [개발자 가이드](README_FOR_DEV.md)를 따릅니다. 개발 에이전트 선택은 플러그인 사용자의 LLM 설정과 별개입니다.

## 모델과 적용 범위

- 모델과 추론 강도는 사용자가 앱이나 CLI에서 선택합니다. 저장소에서는 `model`, `model_reasoning_effort`를 재정의하지 않습니다.
- Astra 기준 최적화는 짧은 시작 지침, 필요한 자료만 읽는 탐색 방식, 실제 코드에 맞는 스킬 구성에 적용합니다. 특정 모델이나 추론 강도를 강제하는 근거로 삼지 않습니다.
- 모델에 관계없이 동일한 테스트·보안·빌드·화면 검수 기준을 적용합니다.
- 전역 모델·MCP·플러그인·승인 정책과 컨텍스트 창·자동 압축 기준·도구 출력 한도를 임의로 바꾸지 않습니다.

새 CLI 작업은 저장소 루트에서 실행합니다.

```fish
codex
```

데스크톱에서는 모델 선택기에서 원하는 모델과 해당 모델이 지원하는 추론 강도를 선택합니다.
...
- 저장소의 모델·추론 강도 재정의를 제거했습니다. 사용자 설정과 작업별 선택을 따릅니다.
