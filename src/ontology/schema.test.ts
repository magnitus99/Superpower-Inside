...
describe('Ontology schema', () => {
  it('단일 기본 schema만 제공하고 도메인 schema 옵션을 노출하지 않는다', () => {
    expect(DEFAULT_ONTOLOGY_SCHEMA.id).toBe('default');
    expect(DEFAULT_ONTOLOGY_SCHEMA.name).toBe('Default');
    expect(DEFAULT_ONTOLOGY_SCHEMA.entityTypes.map((entityType) => entityType.id)).not.toContain(
      'biblical_person',
    );
    expect(
      DEFAULT_ONTOLOGY_SCHEMA.relationTypes.map((relationType) => relationType.id),
    ).not.toContain('quotes_passage');
  });

  it('기본 schema는 검증을 통과한다', () => {
    expect(validateOntologySchema(DEFAULT_ONTOLOGY_SCHEMA)).toEqual([]);
  });

  it('relation source/target type이 domain/range 제약을 만족하면 통과한다', () => {
    const result = validateOntologyRelation(DEFAULT_ONTOLOGY_SCHEMA, {
  ...
  it('unknown relation/type은 reject한다', () => {
    expect(
      validateOntologyRelation(DEFAULT_ONTOLOGY_SCHEMA, {
